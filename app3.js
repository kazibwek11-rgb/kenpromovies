...
function kpLoad(url) {
  kpCurrentUrl = url;
  var container = document.getElementById("player-video");
  container.innerHTML = "";
  var oldBack = document.getElementById("kp-wrap").querySelector("button[data-extra]");
  if (oldBack) oldBack.remove();
  document.getElementById("kp-ctrl").style.display = "";
  kpVideo = null; kpIframe = null; kpIsVideo = false;
  showKpState("loading");
  if (!url) { showKpState("error"); return; }
  var isYT = /youtu\.?be|youtube\.com/i.test(url);
  var isArchive = /archive\.org/i.test(url);
  var isMp4 = /\.(mp4|webm|ogg|m3u8)(\?|$)/i.test(url) || /ia\d+\.us\.archive\.org/i.test(url) || /archive\.org\/download\//i.test(url);
  if (isYT) {
    kpLoadIframe(ytEmbedUrl(url));
  } else if (isArchive && !isMp4) {
    var m = url.match(/archive\.org\/(?:details|embed)\/([^\/?#]+)/i);
    var itemId = m ? m[1] : "";
    var embedUrl = itemId ? ("https://archive.org/embed/" + itemId + "?autoplay=1&playlist=1") : (url.replace("/details/", "/embed/") + (url.includes("?") ? "&" : "?") + "autoplay=1");
