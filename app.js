...
// Get direct mp4 URL from archive.org item
async function getDirectMp4(itemId) {
  try {
    const res = await fetch(`https://archive.org/metadata/${itemId}`);
    const data = await res.json();
    const files = data.files || [];
    const vid = files.find(f => /\.mp4$/i.test(f.name) && f.source !== 'derivative') ||
                files.find(f => /\.mp4$/i.test(f.name)) ||
                files.find(f => /\.mkv$/i.test(f.name));
    if (vid) return `https://archive.org/download/${itemId}/${encodeURIComponent(vid.name)}`;
  } catch(e) {}
  return null;
}
...
async function downloadItem(id) {
  ...
  if (!rawUrl) { showToast('No download link for this movie', true); return; }
  showToast('Preparing download...');
  let finalUrl = rawUrl;
  // If archive.org — get the direct mp4
  const archiveMatch = rawUrl.match(/archive\.org\/(?:details|download|embed)\/([^/?#\/]+)/);
  if (archiveMatch) {
    const itemId = archiveMatch[1];
    const direct = await getDirectMp4(itemId);
    if (direct) {
      finalUrl = direct;
    } else {
      finalUrl = `https://archive.org/download/${itemId}`;
    }
  }
  // Create blob-fetch download to stay in app
  try {
    showToast('Downloading ' + (m.title || 'movie') + '...');
    const a = document.createElement('a');
    a.href = finalUrl;
    a.download = (m.title || 'movie') + '.mp4';
...
function renderDownloadsPage() {
  ...
  if (!list) return;
  // Show all content with download links first, then rest
  const withDl  = allContent.filter(m => m.dl || m.play);
  if (count) count.textContent = withDl.length + ' titles';
  if (!withDl.length) { list.innerHTML = '<div class="empty-page">No movies yet.</div>'; return; }
  list.innerHTML = withDl.map(m => {
    const title = m.epTitle || m.title || 'Unknown';
    const sub   = [m.vj, m.year, m.genre].filter(Boolean).join(' · ');
    const hasDl = !!(m.dl || m.play);
    return `<div class="dl-item">
      <div class="dl-item-thumb">
        ${m.thumb ? `<img src="${m.thumb}" onerror="this.style.display='none'"/>` : ''}
      </div>
      <div class="dl-item-info">
        <div class="dl-item-title">${title}</div>
        <div class="dl-item-sub">${sub}</div>
        ${m.cat === 'series' ? `<span class="dl-item-badge">Series</span>` : ''}
      </div>
      <button class="dl-item-btn ${hasDl ? '' : 'disabled'}" onclick="downloadItem('${m.id}')" ${hasDl ? '' : 'disabled'}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        ${hasDl ? 'Download' : 'No link'}
      </button>
    </div>`;
  }).join('');
}
