...
function downloadGate() {
  ...
  const m = id ? allContent.find(c => c.id === id) : null;
  const dlUrl = m ? (m.dl || m.play) : null;
  if (!dlUrl) { showToast('No download link for this movie', true); return; }
  // Force direct download — create a hidden link with download attribute
  const a = document.createElement('a');
  a.href = dlUrl;
  a.download = (m.title || 'movie') + '.mp4';
  a.target = '_blank';
  // For archive.org, convert details URL to direct download
  if (dlUrl.includes('archive.org/details/')) {
    const itemId = dlUrl.match(/archive\.org\/details\/([^/?#]+)/)?.[1];
    if (itemId) {
      a.href = `https://archive.org/download/${itemId}`;
      a.download = '';
    }
  }
...
// Download from any card (not in player)
function downloadItem(id) {
  const m = allContent.find(c => c.id === id);
  if (!m) return;
  const dlUrl = m.dl || m.play;
  if (!dlUrl) { showToast('No download link for this movie', true); return; }
  const a = document.createElement('a');
  a.href = dlUrl;
  a.download = (m.title || 'movie') + '.mp4';
  ...
  if (dlUrl.includes('archive.org/details/')) {
    const itemId = dlUrl.match(/archive\.org\/details\/([^/?#]+)/)?.[1];
    if (itemId) { a.href = `https://archive.org/download/${itemId}`; a.download = ''; }
  }
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
