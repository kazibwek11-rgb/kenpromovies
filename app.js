...
async function downloadItem(id) {
  ...
  // If it's already a direct .mp4 link — use it as-is
  if (/\.mp4(\?|$)/i.test(rawUrl)) {
    finalUrl = rawUrl;
    const parts = rawUrl.split('/');
    fileName = decodeURIComponent(parts[parts.length-1]) || fileName;
  }
  // archive.org — fetch metadata to get direct mp4 URL
  ...
  // Track in downloads history
  addToDownloadHistory(m, fileName, finalUrl);

  // Show downloads page
  showSection('downloads');

  // Fetch as blob so it downloads without opening new tab
  ...
  try {
    ...
    // Blob fetch failed (CORS) — fallback to direct link
    const a = document.createElement('a');
    a.href = finalUrl;
    a.download = fileName;
    a.target = '_blank';
...
function renderDownloadsPage() {
  ...
  if (!list) return;
  if (count) count.textContent = dlHistory.length ? dlHistory.length + ' item' + (dlHistory.length!==1?'s':'') : '';
  if (!dlHistory.length) {
    list.innerHTML = `<div class="dl-empty">
      ...
      <div class="dl-empty-desc">Click the Download button on any movie or episode to start downloading</div>
    </div>`;
    return;
  }
  list.innerHTML = dlHistory.map(d => {
    ...
    return `<div class="dl-item">
      <div class="dl-item-thumb">
        ...
        <div class="dl-item-status-ico">${isDone ? '✓' : '⬇'}</div>
      </div>
      <div class="dl-item-info">
        <div class="dl-item-title">${d.title||'Unknown'}</div>
        <div class="dl-item-sub">${d.vj} · ${timeStr}</div>
        <div class="dl-item-filename">${d.fileName}</div>
        <span class="dl-status-badge ${isDone?'done':'active'}">${isDone?'Completed':'Downloading'}</span>
      </div>
