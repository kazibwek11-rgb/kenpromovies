...
function buildPlayer(url) {
  ...
  box.innerHTML = '';
  if (!url) {
    box.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:200px;color:#666;font-size:13px;background:#000">No play link set for this movie</div>';
    return;
  }
  ...
  if (url.includes('archive.org')) {
    ...
    if (itemId) {
      ...
      box.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:200px;background:#000;gap:10px"><div style="width:36px;height:36px;border:3px solid #333;border-top-color:#00e5c3;border-radius:50%;animation:spin .8s linear infinite"></div><div style="color:#666;font-size:12px">Loading...</div></div>';
