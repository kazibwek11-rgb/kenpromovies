...
async function downloadItem(id) {
  ...
  let finalUrl = rawUrl;
  let fileName  = (m.title || 'movie').replace(/[^a-z0-9 ]/gi, '') + '.mp4';
  ...
  if (archiveMatch) {
    ...
    const result = await getDirectMp4(itemId);
    if (result) {
      finalUrl = result.url;
  ...
  // Use intent URL on Android for native download manager
  const isAndroid = /android/i.test(navigator.userAgent);
  if (isAndroid) {
    // Android: open direct URL — browser will offer Save/Download
