...
/* Auto-fix partial archive.org URLs e.g. "download/item/file.mp4" → full URL */
function fixUrl(url) {
  if (!url) return '';
  url = url.trim();
  /* Already has a scheme — leave as-is */
  if (/^https?:\/\//i.test(url)) return url;
  /* Looks like an archive.org path without scheme */
  if (/^(download|details|embed)\//.test(url)) return 'https://archive.org/' + url;
  /* Has archive.org host but no scheme */
...
async function submitContent() {
  ...
  /* Build object — NEVER include undefined values (Firestore rejects them) */
  const obj = {
    category: cat,
    title,
  ...
  /* Only add series fields for series — omitting them entirely avoids undefined */
  if (cat === 'series') {
